---
Title: 【Istio⛵️】Istioによるサービスディスカバリー (サービス間通信) の仕組み
Category:
  - Istio
  - Envoy
  - Kubernetes
...
今回は、子守りの前提知識の復習もかねて、サービスメッシュを実装するIstioによるサービスディスカバリーの仕組みを記事で解説しました。

『**サービス間通信の仕組み**』と言い換えると、よりわかりやすいかもしれません。

Istioの機能の1つであるサービスディスカバリーは、その仕組みの多くをEnvoyに頼っているため、合わせてEnvoyの仕組みも説明します。

以下でスライド発表もしておりますため、こちらもどうぞ🙇🏻‍
...
各XDS-APIから個別に宛先情報を取得できる一方で、Envoy上で宛先情報のバージョンの不整合が起こるため、IstioはADS-APIを使用しています。
...
<div class="text-box">
JSONだと見にくいので、<code>yq</code>コマンドでYAMLに変換すると見やすくなります👍
</div>
...
Istioの機能を1つとっても、複雑な仕組みで実現していることがお分かりいただけたかと思います (Istioありがとう🙏)
