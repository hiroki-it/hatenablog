...
なお、セキュリティのベストプラクティスとして、IstiodとGatewayは異なるNamespaceで動かすことが推奨されています。
...
`kubectl get`コマンドを実行し、`istioctl install`コマンドで何をインストールしたのかを確認してみます。
...
`kubectl get`コマンドを実行し、`istioctl uninstall`コマンドで何をアンインストールしたのかを確認してみます。
