...
詳しく知りたい方は、以下の記事で全体像を紹介しており、よろしくどうぞ🙇🏻‍♂️
...
もし、Applicationで無許可の操作 (例：設定値の更新) を実行しようとすると、以下のようなエラーになります。
...
もし、`foo`プロダクトチームが無許可のCluster (例：`bar-cluster`) にマニフェストをデプロイしようとすると、argocd-serverは`destination`のエラーを返却します。
...
### (2) application-controllerがアクセス可能なNamespace取得
