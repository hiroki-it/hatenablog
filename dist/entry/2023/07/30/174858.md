...
もし、Applicationで無許可な操作 (例：設定値の更新) を実行しようとすると、以下のようなエラーになります。
...
もし、`foo`プロダクトチームが無許可なNamespace (例：`bar`) でApplicationを作成しようとすると、argocd-serverは`destination`のエラーを返却します

```sh
application destination {https://kubernetes.default.svc bar} is not permitted in project 'infra-team'
```

もし、`foo`プロダクトチームが無許可なCluster (例：`bar-cluster`) にマニフェストをデプロイしようとすると、argocd-serverは`destination`のエラーを返却します。
...
もし、`foo`プロダクトチームが無許可なNamespace (例：`app`) にマニフェストをデプロイしようとすると、argocd-serverは`destination`のエラーを返却します。
