---
Title: 【Terraform🧑🏻‍🚀】tfstateファイル分割パターンとディレクトリー構成設計
...
- Terraformの`tfstate`ファイルを分割する目的と、分割パターンについて (AWS向け) 
- Terraformのリポジトリやリモートバックエンドのディレクトリー構成の設計について
...
前世でもう少し徳を積んでいれば、[Mitchell Hashimoto](https://twitter.com/mitchellh) として生まれることができたのに！ (適当)
...
そのため、**<font color="#FF0000">基準なしにディレクトリー構成を考えると何でもあり</font>** になってしまいます。

一方で、`tfstate`ファイル分割に基づいて設計することにより、**<font color="#FF0000">明確なディレクトリー構成パターン</font>** として抽出できるようになります。
...
<div class="text-box">
Terraformには、そのリポジトリ内だけで複数のブロックを使い回すことを目的とした、ローカルモジュールがあります。
<br>
<br>
今回、これのディレクトリー構成は設計に含めていません。
<br>
<br>
混同しやすいのですが、<code>tfstate</code>ファイル分割に基づくディレクトリー構成とローカルモジュール内のそれは、全く別のテーマとして切り離して考えることができます👍
<br>
</div>
...
`tfstate`ファイルの分割パターンは、高/中間/低 の層に大別できます。
...
<br>
<br>
例えば、Kubernetesのプロバイダーは、EKSと同じ<code>tfstate</code>ファイルで管理した方が良いです👍
<br>
<blockquote>
...
この分割方法により、各変更頻度グループの管理者が互いに影響を受けずに、`terraform`コマンドの結果を得られるようになります。
