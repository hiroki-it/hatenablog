---
Title: 【Terraform🧑‍🚀】tfstateファイルの分割パターンとディレクトリー構成への適用
Category:
  - Terraform
  - AWS
  - ソフトウェアアーキテクチャ
...
各作業ブランチでは、まるで暗黙的に`target`オプションがついたように、他の作業ブランチから影響を受けずに`terraform`コマンドを実行できます。
...
逆にいえば、運用ルールや開発者人数が理由で`target`オプションが必要ない状況であれば、`tfstate`ファイルは分割しなくてもよいでしょう。
...
その一方で、`tfstate`ファイル分割に基づいて設計することにより、**<font color="#FF0000">明確なディレクトリー構成パターン</font>** として抽出可能になります。
