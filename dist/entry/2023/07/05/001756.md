...
異なるリポジトリに`tfstate`ファイルをおいた方が良い場合については、[分割パターン](#06-tfstateファイルの分割パターン) で説明しています。
...
異なるリモートバックエンドに`tfstate`ファイルをおいた方が良い場合については、[分割パターン](#06-tfstateファイルの分割パターン) で説明しています。
...
**`foo-tfstate`ファイルは`bar-tfstate`ファイルに依存している</font>**
...
分割しすぎると`terraform_remote_state`ブロック地獄になるため、細かすぎず粗すぎない適切な境界を見つけていきましょう。
...
<br>
<br>
特にこの分割方法は、後述のAWSリソースの種類グループとごっちゃになってしまう場合があるため、プロダクトのサブコンポーネントとして意識的に統一させる必要があります👍
</div>
