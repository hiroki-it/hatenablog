...
ここで仮定した状況では、各プロダクトの`tfstate`ファイルは、**<font color="#FF0000">共有network系コンポーネントの tfstate ファイルに依存している</font>** とします。
...
├── remote_state.tf # 他の tfstate ファイルに依存する
├── 📂 tes/ # Tes環境
│    ├── backend.tfvars # Tes環境のfoo-productの terraform.tfstate ファイルを指定する
│    ...
│
├── 📂 stg/ # Stg環境
│    ├── backend.tfvars # Stg環境のfoo-productの terraform.tfstate ファイルを指定する
│    ...
│
└── 📂 prd/ # Prd環境
      ├── backend.tfvars # Prd環境のfoo-productの terraform.tfstate ファイルを指定する
      ...
```
...
├── remote_state.tf # 他の tfstate ファイルに依存する
├── 📂 tes/ # Tes環境
│    ├── backend.tfvars # Tes環境のbar-productの terraform.tfstate ファイルを指定する
│    ...
│
├── 📂 stg/ # Stg環境
│    ├── backend.tfvars # Stg環境のbar-productの terraform.tfstate ファイルを指定する
│    ...
│
└── 📂 prd/ # Prd環境
      ├── backend.tfvars # Prd環境のbar-productの terraform.tfstate ファイルを指定する
      ...
```
...
├── vpc.tf
├── 📂 tes/ # Tes環境
│    ├── backend.tfvars # Tes環境のnetwork系コンポーネントの状態を持つ terraform.tfstate ファイルを指定する
│    ...
│
├── 📂 stg/ # Stg環境
│    ├── backend.tfvars # Stg環境のnetwork系コンポーネントの状態を持つ terraform.tfstate ファイルを指定する
│    ...
│
└── 📂 prd/ # Prd環境
      ├── backend.tfvars # Prd環境のnetwork系コンポーネントの状態を持つ terraform.tfstate ファイルを指定する
      ...
```
...
🪣 tes-bucket/
├── 📂 foo-product
│    └── terraform.tfstate # foo-product の状態を持つ
│
├── 📂 bar-product
│    └── terraform.tfstate # bar-product の状態を持つ
│
└── 📂 network
      └── terraform.tfstate #network系コンポーネントの状態を持つ
```
...
├── s3.tf
├── 📂 tes/ # Tes環境
│    ├── backend.tfvars # Tes環境のfrontendチームの状態を持つ terraform.tfstate ファイルを指定する
│    ...
│
├── 📂 stg/ # Stg環境
│    ├── backend.tfvars # Stg環境のfrontendチームの状態を持つ terraform.tfstate ファイルを指定する
│    ...
│
└── 📂 prd/ # Prd環境
      ├── backend.tfvars # Prd環境のfrontendチームの状態を持つ terraform.tfstate ファイルを指定する
      ...
```
...
├── rds.tf
├── 📂 tes
│    ├── backend.tfvars # Tes環境のbackendチームの状態を持つ terraform.tfstate ファイルを指定する
│    ...
│
├── 📂 stg
│    ├── backend.tfvars # Stg環境のbackendチームの状態を持つ terraform.tfstate ファイルを指定する
│    ...
│
└── 📂 prd
      ├── backend.tfvars # Prd環境のbackendチームの状態を持つ terraform.tfstate ファイルを指定する
       ...
```
...
├── vpc.tf
├── 📂 tes
│    ├── backend.tfvars # Tes環境のsreチームの terraform.tfstate ファイルを指定する
│    ...
│
├── 📂 stg
│    ├── backend.tfvars # Stg環境のsreチームの terraform.tfstate ファイルを指定する
│    ...
│
└── 📂 prd
      ├── backend.tfvars # Prd環境のsreチームの terraform.tfstate ファイルを指定する
      ...
```
...
│
└── 📂 sre-team
      └── terraform.tfstate # sre チームの状態を持つ
```
...
│
├── 📂 cicd
│    └── terraform.tfstate # cicd コンポーネントの状態を持つ
│
├── 📂 datastore
│    └── terraform.tfstate # datastore コンポーネントの状態を持つ
│
├── 📂 monitor
│    └── terraform.tfstate #monitorコンポーネントの状態を持つ
│
...
│    ├── ses.tf
│    ├── 📂 tes/ # Tes環境
│    │    ├── backend.tfvars # Tes環境のfrontendチームの application/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    ├── 📂 stg/ # Stg環境
│    │    ├── backend.tfvars # Stg環境のfrontendチームの application/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    └── 📂 prd/ # Prd環境
│          ├── backend.tfvars # Prd環境のfrontendチームの application/terraform.tfstate ファイルを指定する
│          ...
│
└── 📂 monitor/
      ├── provider.tf
      ...
      ├── cloudwatch.tf
      ├── 📂 tes/ # Tes環境
      │    ├── backend.tfvars # Tes環境のfrontendチームの monitor/terraform.tfstate ファイルを指定する
      │    ...
      │
      ├── 📂 stg/ # Stg環境
      │    ├── backend.tfvars # Stg環境のfrontendチームの monitor/terraform.tfstate ファイルを指定する
      │    ...
      │
      └── 📂 prd/ # Prd環境
            ├── backend.tfvars # Prd環境のfrontendチームの monitor/terraform.tfstate ファイルを指定する
            ...
```
...
│    ├── sns.tf
│    ├── 📂 tes/ # Tes環境
│    │    ├── backend.tfvars # Tes環境のbackendチームの application/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    ├── 📂 stg/ # Stg環境
│    │    ├── backend.tfvars # Stg環境のbackendチームの application/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    └── 📂 prd/ # Prd環境
│          ├── backend.tfvars # Prd環境のbackendチームの application/terraform.tfstate ファイルを指定する
│          ...
│
└── 📂 monitor/
      ├── provider.tf
      ...
      ├── cloudwatch.tf
      ├── 📂 tes/ # Tes環境
      │    ├── backend.tfvars # Tes環境のbackendチームの monitor/terraform.tfstate ファイルを指定する
      │    ...
      │
      ├── 📂 stg/ # Stg環境
      │    ├── backend.tfvars # Stg環境のbackendチームの monitor/terraform.tfstate ファイルを指定する
      │    ...
      │
      └── 📂 prd/ # Prd環境
            ├── backend.tfvars # Prd環境のbackendチームの monitor/terraform.tfstate ファイルを指定する
            ...
```
...
│    ├── eks.tf
│    ├── 📂 tes/ # Tes環境
│    │    ├── backend.tfvars # Tes環境のsreチームの application/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    ├── 📂 stg/ # Stg環境
│    │    ├── backend.tfvars # Stg環境のsreチームの application/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    └── 📂 prd/ # Prd環境
│          ├── backend.tfvars # Prd環境のsreチームの application/terraform.tfstate ファイルを指定する
│          ...
│
├── 📂 auth/
│    ├── provider.tf
...
│    ├── iam.tf
│    ├── 📂 tes/ # Tes環境
│    │    ├── backend.tfvars # Tes環境のsreチームの auth/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    ├── 📂 stg/ # Stg環境
│    │    ├── backend.tfvars # Stg環境のsreチームの auth/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    └── 📂 prd/ # Prd環境
│          ├── backend.tfvars # Prd環境のsreチームの auth/terraform.tfstate ファイルを指定する
│          ...
│
├── 📂 monitor/
│    ├── provider.tf
...
│    ├── cloudwatch.tf
│    ├── 📂 tes/ # Tes環境
│    │    ├── backend.tfvars # Tes環境のsreチームの monitor/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    ├── 📂 stg/ # Stg環境
│    │    ├── backend.tfvars # Stg環境のsreチームの monitor/terraform.tfstate ファイルを指定する
│    │    ...
│    │
│    └── 📂 prd/ # Prd環境
│          ├── backend.tfvars # Prd環境のsreチームの monitor/terraform.tfstate ファイルを指定する
│          ...
│
└── 📂 network
      ├── provider.tf
      ...
      ├── vpc.tf
      ├── 📂 tes/ # Tes環境
      │    ├── backend.tfvars # Tes環境のsreチームの network/terraform.tfstate ファイルを指定する
      │    ...
      │
      ├── 📂 stg/ # Stg環境
      │    ├── backend.tfvars # Stg環境のsreチームの network/terraform.tfstate ファイルを指定する
      │    ...
      │
      └── 📂 prd/ # Prd環境
            ├── backend.tfvars # Prd環境のsreチームの network/terraform.tfstate ファイルを指定する
            ...
```
...
└── 📂 sre-team
      ...
      │
      └── 📂 network
            └── terraform.tfstate # sreチームのnetwork系コンポーネントの状態を持つ
```
