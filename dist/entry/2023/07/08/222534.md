...
もし、fooプロダクトチームが無許可Namespace (例：`bar`) でApplicationを作成しようとすると、argocd-serverは`destination`の認可エラーを返却します
...
もし、fooプロダクトチームがApplicationで無許可Cluster (例：`bar-cluster`) をデプロイ先として指定しようとすると、argocd-serverは`destination`の認可エラーを返却します。
...
もし、fooプロダクトチームが無許可Namespace (例：`app`) をデプロイ先に指定しようとすると、argocd-serverは`destination`の認可エラーを返却します。
