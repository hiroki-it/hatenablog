...
プロダクトチームは、認可されたテナント内のApplicationのみを操作できます。

これにより、誤ったプロダクト用Clusterにマニフェストをデプロイすることを制限できます。
...
また、ArgoCD専用のNamespace (argocd) も用意します。
...
CLモードなArgoCDは任意のNamespaceのApplicationにアクセスできるため、NSモードとは異なり、`.spec.sourceNamespaces`キーでマニフェストのデプロイを制限できます。

`.spec.sourceNamespaces`キーで、特定のNamespaceのApplicationがこのAppProjectに所属できないように、ApplicationのNamespaceを制限しています。
...
CLモードなArgoCDは任意のNamespaceのApplicationにアクセスできるため、NSモードとは異なり、`--application-namespaces`オプションにアクセス可能なNamespaceを設定する必要があります。
...
NSモードと同様にして、`argocd-rbac-cm`では、Applicationを操作するログインユーザーが、無認可のAppProjectに所属するApplicationを操作できないように制限します。
...
認証済みのプロダクトチーム (foo-team、bar-team、baz-team) に対して、無認可のAppProject (foo、bar、baz) に所属するApplicationを操作できないように、認可スコープで制限しています。
...
data:
  ...
  policy.csv: |
    ...
    p, role:<ロール名>, <Kubernetesリソースの種類>, <アクション名>, <AppProject名>/<Namespace名>/<Kubernetesリソースの識別名>, <許否>
...
NSモードなArgoCDは自身が所属するNamespaceのApplicationのみにアクセスできるため、CLモードとは異なり、`.spec.sourceNamespaces`キーでマニフェストのデプロイを制限する必要はありません。
...
NSモードなArgoCDは自身が所属するNamespaceのApplicationのみにアクセスできるため、CLモードとは異なり、`.application.namespaces`キーでアクセス可能なNamespaceを制限する必要はありません。
...
CLモードと同様にして、`argocd-rbac-cm`では、ログインユーザーが無認可のProjectテナントに属するApplicationを操作できないようにします。
...
|      安全性<br>(セキュリティ)       | ログインユーザー用ConfigMap (`argocd-rbac-cm`) を使用すれば、無認可の実行環境AppProjectに所属するApplicationを操作できないように制限できる。 | -                                                                                                                                         | ➡︎ | -                                                                                         |
...
|      安全性<br>(セキュリティ)       | ログインユーザー用ConfigMap (`argocd-rbac-cm`) を使用すれば、無認可の実行環境AppProjectを操作できないように制限できる。 | -                                                                                                                                                    | ➡︎ | -                                                                                   |
...
| :---------------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ----------------------------------------------------------------------------------- |
|               拡張性                | ArgoCDのPod数が多くなり、将来的にNode当たりのPodやIPアドレスの上限数にひっかかりにくい。                                                       | -                                                                                                                                                    | ➡︎ | -                                                                                   |
|      安全性<br>(セキュリティ)       | ログインユーザー用ConfigMap (`argocd-rbac-cm`) を使用すれば、無認可のサブチームAppProjectに所属するApplicationを操作できないように制限できる。 | -                                                                                                                                                    | ➡︎ | -                                                                                   |
|               保守性                | -                                                                                                                                              | 異なる実行環境に関するApplicationが共存しており、同じargocd-server (ダッシュボード) から操作することになるため、実行環境間の選択ミスが起こりやすい。 | ➡︎ | ダッシュボードにはApplicationのフィルタリング機能があるため、選択ミスを回避できる。 |
...
Projectテナントは、ログインユーザーが無認可のNamespaceやClusterにマニフェストをデプロイすることを制限します。
...
(1) argocd-serverは、`argocd-cmd-params-cm`から自身がアクセス可能なNamespaceを取得します。

(2) fooプロダクトチームのinfraチームが、argocd-serverを操作します。

(3) argocd-serverは、`argocd-rbac-cm`からApplication操作に関する認可スコープを取得します

(4) infraチームは、認可されたApplicationを操作します。

(5) infraチームは、fooプロダクト用のdev Clusterの`infra`というNamespaceにマニフェストをデプロイできます。

### (エラー例1) 無認可のNamespaceでApplicationを作成しようとした場合

例えば、fooプロダクトチームが無認可のNamespace (例：`bar`) でApplicationを作成しようとします。
...
### (エラー例2) 無認可のAppProjectでApplicationを作成しようとした場合

例えば、fooプロダクトチームが無認可のAppProject (例：`bar-team`) でApplicationを作成しようとします。
...
### (エラー例3) 無認可のプロダクト用Clusterを指定しようとした場合

例えば、fooプロダクトチームがApplicationを操作し、無認可のプロダクト用Cluster (例：`bar-cluster`) をデプロイ先として指定しようします。
...
### (エラー例4) 無認可のNamespaceをデプロイ先に指定しようとした場合

例えば、fooプロダクトチームがApplicationを操作し、無認可のNamespace (例：`app`) をデプロイ先に指定しようします。
...
## 無認可テナントへのReconciliation制限

Projectテナントは、application-controllerが無認可のApplicationにReconciliationを実行することを制限します。
...
例えば、application-controllerが、無認可の空間に所属するArgoCD系カスタムリソースに対してReconciliationを実行しようとします。
