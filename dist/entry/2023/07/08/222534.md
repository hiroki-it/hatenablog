...
<table>
<tr style="text-align: center;"><th>テナント<br>分割パターン</th><td>実Clusterテナント</td><td>仮想Clusterテナント</td><td>Namespaceテナント</td><td>AppProjectテナント</td></tr>
...
Namespaceテナントを採用した場合、そのNamespaceのどのような粒度で作成するべきでしょうか。

例えば、以下のような複数のプロダクト用Clusterがいる状況と仮定します。

検証環境別のプロダクト用Cluster (例：Tes-1環境、Tes-2環境)
