...
認証済みグループ (foo-team、bar-team、baz-team) に対して、無認可のAppProject (`foo`、`bar`、`baz`) に所属するApplicationを操作できないように、認可スコープで制限しています。
...
認証済みグループ (app-team、infra-team) に対して、無認可のAppProject (`app`、`infra`) に所属するApplicationを操作できないように、認可スコープで制限しています。
...
Projectテナントがマニフェストのデプロイをどのように制限するのかについて、エラー⚠️を挙げて解説します。
...
Projectテナントは、以下のようなエラー⚠️で、マニフェストのデプロイを制限します。
...
## ArgoCD系カスタムリソースのReconciliation制限

プロダクトの実行環境 (Dev環境、Tes環境) 別に管理されたClusterがいる状況と仮定します。

プロダクト別にNamespace (`foo`) 、プロダクトのサブチーム別にAppProject (`app`、`infra`) を用意します。

Projectテナントは、以下のようなエラー⚠️で、ArgoCD系カスタムリソースのReconciliationを制限します。
...
(1) application-controllerは、`argocd-cmd-params-cm`から自身がアクセス可能なNamespaceを取得します。
...
<div class="text-box">
Projectテナントは、その他にも、例えば以下のようなアクセスを制限を設定できます。
<br>
<br>
