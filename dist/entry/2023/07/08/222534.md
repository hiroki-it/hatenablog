...
Projectテナントは二重のテナント (**<font color="#FF0000">第一テナントにNamespace</font>**、**<font color="#FF0000">第二テナントに複数のAppProject</font>**) を持ち、両方のテナントでアクセスを制御します。

AppProjectはNamespaceスコープなカスタムリソースであり、自身に所属するApplicationを一括して制御します。
...
前述のNSモードとの違いとして、`.spec.sourceNamespaces`キーの設定が必要です。
...
## 無許可ProjectテナントへのReconciliation制御
...
Projectテナントは、Applicationに関して、デプロイを許可するKubernetesリソース (`.spec.clusterResourceWhitelist`キー) やポーリングを許可するリポジトリ (`.spec.sourceRepos`キー) を制御できます。

**"無許可Projectテナントへのアクセス"** という観点でテーマが異なるため、本記事では言及しませんでした。
