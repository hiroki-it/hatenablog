...
実Clusterテナントでは、デプロイ先のプロダクト用ClusterごとにArgoCD用の実Clusterを作成します。

この時の実Clusterをテナントの単位とします。

実Clusterテナントは、ArgoCDの各コンポーネントやApplicationを分離します。
...
仮想Clusterテナントは、ArgoCDの各コンポーネントやApplicationを分離します。
