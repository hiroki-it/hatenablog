...
Namespaceテナントでは、テナントごとにNamespaceを作成しつつ、ArgoCDは共有します。
...
AppProjectテナントでは、単一のNamespace内にArgoCDテナントを作成します。

具体的には、プロダクト共有Cluster上に単一のNamespaceを作成します。

また、Namespaceにプロダクト用ClusterごとのAppProjectを作成します。
...
Namespaceテナント間では、ClusterのKubernetesリソースを共有しますが、AppProject内のApplicationは共有しません。
