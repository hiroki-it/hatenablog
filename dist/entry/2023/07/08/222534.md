---
Title: 【ArgoCD🐙️】KubernetesのマルチテナントパターンとArgoCDに適したテナント設計
Category:
  - ArgoCD
  - Kubernetes
  - ソフトウェアアーキテクチャ
...
- ArgoCDでマルチテナントが必要な目的と、適したテナントパターン
- 各テナント内での認可処理の仕組み
...
今回は、この時に整理したKubernetesのマルチテナントパターンとArgoCDに適したテナント設計を記事で解説しました。
...
まず最初に、Kubernetesとしてのマルチテナントパターンを整理しておきます。
...
<table>
   <thead>
      <tr>
         <th style="text-align: center;">マルチテナント<br>パターン名</th>
         <th style="text-align: center;"><nobr>テナントの単位</nobr></th>
         <th>ツール<br>(アルファベット順)</th>
      </tr>
   </thead>
   <tbody>
      <tr>
         <td style="text-align: center;">Clusters-as-a-Service<br>モデル</td>
         <td style="text-align: center;">実Cluster<br>テナント</td>
         <td>Clusterを増やすだけなのでツール不要</td>
      </tr>
      <tr>
         <td style="text-align: center;">Control-planes-as-a-Service<br>モデル</td>
         <td style="text-align: center;">仮想Cluster<br>テナント</td>
         <td>kcp、tensile-kube、vcluster、virtual cluster、など</td>
      </tr>
      <tr>
         <td style="text-align: center;">Namespaces-as-a-Service<br>モデル</td>
         <td style="text-align: center;">Namespace<br>テナント</td>
         <td>Namespaceを増やすだけなのでツール不要</td>
      </tr>
      <tr>
         <td style="text-align: center;">各種ツールの独自モデル</td>
         <td style="text-align: center;">カスタムリソース<br>テナント</td>
         <td>capsule、kiosk、kubezoo、など</td>
      </tr>
   </tbody>
</table>
...
<br>
<br>
この分類方法では、テナント間の分離度に応じて、各マルチテナントを種別します。
<br>
<br>
...
## Clusters-as-a-Serviceモデル

テナントごとに、独立したClusterを提供する。

<br>

## Control-planes-as-a-Serviceモデル

テナントごとに、独立したコントロールプレーンを提供する。

<br>

## Namespaces-as-a-Serviceモデル
...
Kubernetesでのマルチテナントパターンを解説しました。

ここからは、実際にArgoCDのマルチテナント設計で候補になるパターンとその実装方法を解説していきます。
...
<table>
  <thead style="text-align: center;">
    <tr>
      <th rowspan="2">マルチテナント<br>パターン名</th>
      <th rowspan="2"><nobr>テナントの単位</nobr></th>
      <th rowspan="2">テナントの実装</th>
      <th colspan="2"><nobr>テナント間のArgoCD関連リソース分離</nobr><br>(分離できていれば <code>⭕️</code> )</th>
      <th rowspan="2"><nobr>採用</nobr></th>
    </tr>
    <tr>
      <th><nobr>Namespacedスコープ</nobr><br>リソース</th>
      <th><nobr>Clusterスコープ</nobr><br>リソース</th>
    </tr>
  </thead>
  <tbody style="text-align: center;">
    <tr>
      <td>Clusters-as-a-Service<br>モデル</td>
    ...
    <tr>
      <td>Control-planes-as-a-Service<br>モデル</td>
    ...
    <tr>
      <td rowspan="2">Namespaces-as-a-Service<br>モデル</td>
      <td>Namespace<br>テナント</td>
      <td>プロダクト共有Cluster上<br>Namespace</td>
      <td><code>⭕️</code></td>
      <td></td>
      <td>✅</td>
    </tr>
    <tr>
      ...
      <td>プロダクト共有Cluster上<br>AppProject</td>
      <td><code>⭕️</code></td>
      <td></td>
      <td></td>
    </tr>
...
# 04. Clusters-as-a-Serviceモデルの場合
...
|               保守性                | -                                                          | 複数のClusterを継続的に保守しないといけない。<nobr>(例：アップグレード、機能修正、など)</nobr> |
...
# 05. Control-planes-as-a-Serviceモデルの場合
...
# 06. Namespaces-as-a-Serviceモデルの場合

## Namespaceテナント

### Namespaceテナントとは

Namespaceテナントでは、テナントごとにNamespaceを作成します。

具体的には、プロダクト共有Cluster上にプロダクト別のNamespaceを作成し、各Namespace内にのみ認可スコープを持つArgoCDを動かします。
...
具体的には、プロダクト共有Cluster上にプロダクト別のNamespaceを作成し、Cluster全体に認可スコープを持つ単一のArgoCDを動かします。
...
KubernetesのマルチテナントパターンとArgoCDに適したテナント設計をもりもり布教しました。
