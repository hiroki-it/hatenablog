...
| ----------------------------------- | ---------------- | --------------------------------------------------------------------------- |
| Namespacedスコープモード (NSモード) | 各テナントで独立 | ArgoCDの各コンポーネントは、自分自身が属するNamespace内のみにアクセスできる |
| Clusterスコープモード (CLモード)    | 各テナントで共有 | ArgoCDの各コンポーネントは、許可された任意のNamespaceにアクセスできる       |
