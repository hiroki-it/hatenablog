...
- NamespacedスコープモードなArgoCDが無許可テナントへのアクセスを防ぐ仕組み
...
また、ArgoCDの各コンポーネントは自分自身が属するNamespace内のみにアクセスできます。
...
# 05-02. 高分離なAppProjectテナントが無許可テナントへのアクセスを防ぐ仕組み
...
認可エラー例をいくつか挙げて、AppProjectテナントが無許可テナント (他のテナントのNamespace、AppProject、デプロイ先Cluster) への様々なアクセスを防ぐ仕組みを簡単に解説します。

<br>

## ログインユーザーの無許可テナントへのアクセスを防ぐ

AppProjectテナントは、ログインユーザーが無許可テナントでマニフェストを操作することを防ぎます。
...
(2) `foo`プロダクトチームの`infra`チームが、argocd-serverを操作します。

(3) argocd-serverは、`argocd-rbac-cm`から`foo`プロダクトチームのArgoCD系カスタムリソースに対する認可スコープを取得します

(4) `infra`チームは、テナント内で許可されたApplicationを操作します。

(5) `infra`チームは、fooプロダクト用のdev Clusterの`infra`というNamespaceにマニフェストをデプロイします。

### (例1) 無許可NamespaceでApplicationを作成しようとした場合

もし、`foo`プロダクトチームが無許可Namespace (例：`bar`) でApplicationを作成しようとすると、argocd-serverは`destination`の認可エラーを返却します

仮に無許可NamespaceでApplicationを作れてしまうと、そのApplicationから、誤ったClusterにマニフェストをデプロイできてしまいます😈
...
### (例2) 無許可Clusterをデプロイ先に指定しようとした場合

もし、`foo`プロダクトチームがApplicationで無許可Cluster (例：`bar-cluster`) をデプロイ先として指定しようとすると、argocd-serverは`destination`の認可エラーを返却します。

仮に無許可Clusterをデプロイ先に指定できてしまうと、誤ったClusterにマニフェストをデプロイできてしまいます😈
...
### (例3) 無許可Namespaceをデプロイ先に指定しようとした場合

もし、`foo`プロダクトチームが無許可Namespace (例：`app`) をデプロイ先に指定しようとすると、argocd-serverは`destination`の認可エラーを返却します。

仮に無許可Namespaceをデプロイ先に指定できてしまうと、誤ったNamespaceにマニフェストをデプロイできてしまいます😈
...
## application-controllerの無許可テナントへのReconciliationを防ぐ

AppProjectテナントは、application-controllerが無許可テナントにReconciliationすることを防ぎます。
...
(2) application-controllerは、自身が属する同じNamespace内のArgoCD系カスタムリソースに対して、Reconciliationを実行します。

### (例1)

もし、applocation-controllerが無許可Namespace内のArgoCD系カスタムリソースに対してReconciliationを実行しようとすると、内部エラーになります。

> - [https://github.com/argoproj/argo-cd/blob/v2.7.10/controller/appcontroller_test.go#L1462-L1488:title]
