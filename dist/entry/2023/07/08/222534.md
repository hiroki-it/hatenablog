...
エラー例をいくつか挙げて、AppProjectテナントが無許可テナント (他のテナントのNamespace、AppProject、デプロイ先Cluster) への様々なアクセスを防ぐ仕組みを簡単に解説します。
...
### (例1) 無許可Namespace内でApplicationを作成しようとした場合

もし、fooプロダクトチームが無許可Namespace (例：`bar`) 内でApplicationを作成しようとすると、argocd-serverは以下のようなエラーを返却します
...
### (例2) 無許可AppProject内でApplicationを作成しようとした場合

もし、fooプロダクトチームが無許可AppProject (例：`bar-team`) 内でApplicationを作成しようとすると、argocd-serverは以下のようなエラーを返却します。

```sh
Application referencing project 'bar-team' which does not exist
```

### (例3) 無許可Clusterをデプロイ先に指定しようとした場合

もし、fooプロダクトチームがApplicationで無許可Cluster (例：`bar-cluster`) をデプロイ先として指定しようとすると、argocd-serverは以下のようなエラーを返却します。
...
### (例4) 無許可Namespaceをデプロイ先に指定しようとした場合

もし、fooプロダクトチームがApplicationで無許可Namespace (例：`app`) をデプロイ先に指定しようとすると、argocd-serverは以下のようなエラーを返却します。
