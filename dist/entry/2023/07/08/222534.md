...
<table>
<tr style="text-align: center;"><th>マルチテナント<br>パターン</th><td>実Clusterテナント</td><td>仮想Clusterテナント</td><td>Namespaceテナント</td><td>AppProjectテナント</td></tr>
<tr style="text-align: center;"><th>説明</th><td>実際のCluster</td><td>プロダクト共有Cluster上<br>仮想Cluster</td><td>プロダクト共有Cluster上<br>Namespace</td><td>プロダクト共有Cluster上<br>AppProject</td></tr>
<tr style="text-align: center;"><th>分離度</th><td>高</td><td>高</td><td>中</td><td>低</td></tr>
<tr style="text-align: center;"><th>テナント間<br>共有K8sリソース</th><td>なし</td><td>なし</td><td>クラスタースコープ<br>K8sリソース</td><td>クラスタースコープ<br>K8sリソース</td></tr>
<tr style="text-align: center;"><th>Cluster運用<br>つらさ</th><td>とてもつらい</td><td>ふつう</td><td>ふつう</td><td>ふつう</td></tr>
<tr style="text-align: center;"><th>採用</th><td></td><td></td><td>✅</td><td></td></tr>
</table>
...
それでは、ArgoCDのNamespaceテナントを採用する場合に、各Namespaceをどのような粒度で作成するべきなのかを考えていきましょう。

ArgoCDのNamespaceテナントを採用した場合、各Namespaceをどのような粒度で作成するべきでしょうか。

以下のような粒度例があります。

状況に応じて適切な粒度を採用するとよいとのことです。
...
例えば、複数のプロダクト用Clusterがおり、実行環境 (例：Tes-1環境、Tes-2環境) 別にClusterがいると仮定します。

複数のプロダクトの実行環境別にNamespaceを作成します。

![argocd_tenant_namespace_product-environments](https://raw.githubusercontent.com/hiroki-it/tech-notebook-images/master/images/drawio/blog/argocd/argocd_tenant_namespace_product-environments.png)
...
同様にして、複数のプロダクト用Clusterがおり、実行環境 (例：Tes-1環境、Tes-2環境) 別にClusterがいると仮定します。

複数のプロダクト別にNamespaceを作成します。

![argocd_tenant_namespace_products](https://raw.githubusercontent.com/hiroki-it/tech-notebook-images/master/images/drawio/blog/argocd/argocd_tenant_namespace_products.png)
