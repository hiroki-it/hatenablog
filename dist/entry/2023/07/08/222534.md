...
同様にして、プロダクトごとに異なる管理チームがおり、プロダクトの実行環境 (Tes環境、Stg環境) 別にClusterがいると仮定します。

この時、プロダクト管理チーム別にNamespaceを作成します。
...
プロダクト管理チームは、SSOでargocd-serverにログインします。

### (2) argocd-serverによる認可スコープ制御

argocd-serverは、プロダクト管理チームのApplicationの認可スコープを制御します。

なお各プロダクトのArgoCDのApplicationは、プロダクトの実行環境別のClusterに対応しています。

### (3) application-controllerによるマニフェストデプロイ

プロダクト管理チームは、各Namespace上のArgoCD (application-controller) を介して、担当するClusterにのみマニフェストをデプロイできます。
...
プロダクト管理チームがダッシュボード (argocd-server) にSSOを使用してログインしようとします。
