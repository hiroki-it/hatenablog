...
すなわち、Projectテナントは二重のテナント (**<font color="#FF0000">第一テナントにNamespace</font>**、**<font color="#FF0000">第二テナントに複数のAppProject</font>**) を持ち、両方のテナントでアクセスを制御します。
...
CLモードなArgoCDは、全ての許可Namespaceにアクセスできます。
...
NSモードと同様にして、Applicationを操作するログインユーザーが、無許可なProjectテナントをデプロイ先に指定できないように設定できます。

例えば、以下のようになります。

AppProjectに所属するApplicationがfooテナント以外にデプロイ先に指定できないように、`.spec.destination`キーや`.spec.sourceNamespaces`キーを設定しています。
...
`.spec.sourceNamespaces`キーで、このAppProjectへの所属を許可したいApplicationのNamespaceを設定します。
...
全てのNamespaceにアクセスできるように、`*` (アスタリスク) を設定しています。
...
NSモードとの違いとして、`.application.namespaces`キーで、argocd-serverとapplication-controllerがいずれのNamespaceにアクセスできるかを設定する必要があります。

または`argocd-cmd-params-cm`の代わりに、例えば以下のようにPodに引数を直接渡しても良いです。

例えば、以下のようになります。

全てのNamespaceを許可するために、`*` (アスタリスク) を設定しています。
...
NSモードと同様にして、`argocd-rbac-cm`では、ログインユーザーが、無許可なProjectテナントに所属するApplicationを操作できないようにします。

例えば、以下のようになります。

認証済みのプロダクトチーム (foo-team、bar-team、baz-team) に対して、特定のAppProject (foo、bar、baz) に所属するApplicationのみを操作できるように、認可スコープを設定しています。
...
NSモードなArgoCDは、自身が所属するNamespaceのみにアクセスできます。
...
CLモードと同様にして、Applicationを操作するログインユーザーが、無許可なProjectテナントをデプロイ先に指定できないように設定できます。

例えば、以下のようになります。

AppProjectに所属するApplicationが、fooテナント以外にデプロイ先に指定できないように、`.spec.destination`キーを設定しています。
