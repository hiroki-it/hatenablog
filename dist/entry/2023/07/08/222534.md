...
プロダクトチームがApplicationを操作する時に、テナントは認可されたテナントに所属するApplicationのみを操作させるように制限します。
...
例えば、application-controllerが、無認可のNamespaceに所属するArgoCD系カスタムリソースに対して、Reconciliationを実行しようとします。

すると、application-controllerは内部で検証メソッドを実行し、Reconciliationを回避しようとします。

ただし、ログインユーザーがapplication-controllerを手動で操作することは基本的に無いため、エラーを知ることはありません。
