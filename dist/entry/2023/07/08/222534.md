...
例では、AppProjectに所属するApplicationがfooテナント以外にデプロイ先に指定できないようにしています。
...
全てのNamespaceを許可したい場合、`*` (アスタリスク) を設定します。
...
NSモードと同様にして、`argocd-rbac-cm`では、ログインユーザーが、無許可なProjectテナントに属するApplicationを操作できないようにします。
...
例では、AppProjectに所属するApplicationが、fooテナント以外にデプロイ先に指定できないようにしています。
