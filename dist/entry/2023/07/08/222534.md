...
ただしArgoCDが増えていくにつれて、同じ種類のコンポーネント (例：argocd-server、application-controller、repo-server、など) 間で何かしらの問題 (例：Kubernetesリソース名の衝突、処理の干渉、など) が起こる可能性があります。
...
<table>
   <thead>
      <tr>
         <th>テナント分割パターン</th><th>説明</th><th><nobr>採用</nobr><br>・<br><nobr>不採用</nobr></th>
      </tr>
   </thead>
   <tbody>
      <tr>
         <td>実Cluster単位</td><td>実際の各Clusterをテナントとする。</td><td rowspan="3">不採用</td>
      </tr>
      <tr>
         <td>仮想Cluster単位</td>
      ...
      <tr>
         <td>Namespace単位</td>
         <td>プロダクト共有Cluster上の各Namespaceをテナントとする。</td><td>採用</td>
      </tr>
   </tbody>
</table>
...
また、それぞれのNamespaceにプロダクト用ClusterごとのAppProjectを作成します。
