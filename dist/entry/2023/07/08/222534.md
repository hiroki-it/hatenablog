...
AppProjectテナントは、無許可AppProjectテナント (Namespace、AppProject) やこれのデプロイ先Clusterへのアクセスを防ぎます。

エラー例をいくつか挙げて解説します⚠️

<br>

## ログインユーザーの無許可テナント内でのマニフェスト操作を防ぐ

AppProjectテナントは、ログインユーザーが無許可テナント内でマニフェストを操作することを防ぎます。
...
### (例1) 無許可テナント内のNamespaceでApplicationを作成しようとした場合

例えば、fooプロダクトチームが無許可テナント内のNamespace (例：`bar`) 内でApplicationを作成しようとします。
...
無許可テナント内のNamespaceでApplicationを作れてしまうと、そのApplicationから誤ったClusterにマニフェストをデプロイできてしまいます😈

> - [https://github.com/argoproj/argo-cd/blob/v2.7.10/util/argo/argo_test.go#L679-L710:title]

### (例2) 無許可テナント内のAppProjectでApplicationを作成しようとした場合

もし、fooプロダクトチームが無許可テナント内のAppProject (例：`bar-team`) でApplicationを作成しようとします。
...
無許可テナント内のAppProjectでApplicationを作成できてしまうと、そのApplicationから誤ったClusterにマニフェストをデプロイできてしまいます😈

### (例3) 誤ったClusterをデプロイ先に指定しようとした場合
...
### (例4) 無許可テナント内のNamespaceをデプロイ先に指定しようとした場合

例えば、fooプロダクトチームが、Applicationで無許可テナント内のNamespace (例：`app`) をデプロイ先に指定しようします。
...
無許可テナント内のNamespaceをデプロイ先に指定できてしまうと、そのApplicationから誤ったNamespaceにマニフェストをデプロイできてしまいます😈
...
例えば、application-controllerが、無許可テナント内のNamespaceに属するArgoCD系カスタムリソースに対してReconciliationを実行しようとします。
