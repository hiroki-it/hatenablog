...
- [AppProjectテナントの実践例](#05-NSモードなArgoCDのAppProjectテナントの実践例) (5章)
...
# 05. NSモードなArgoCDのAppProjectテナントの実践例

## AppProjectテナント実践の一覧

今度は、NSモードなArgoCDのAppProjectテナントを採用する場合、おすすめのAppProject実践例を解説していきます。

前述の通り、NSモードなArgoCDでは、NamespaceとAppProjectの両方のテナントパターンを考える必要があります。

<table>
  <thead>
  <tr>
    <th rowspan="2" style="text-align: center;"></th>
    <th colspan="2" style="text-align: center;">AppProject実践例<br>(NamespaceとAppProjectの組み合わせ)</th>
...
|               安全性                | 専用ConfigMap (`argocd-rbac-cm`) を使用すれば、各テナント内で利用者に実行環境別の認可スコープを設定できる。 | -                                                                                                                         |
...
また、ArgoCDでのテナントパターンの実践にあたり、以下の方に有益なプラクティスをご教授いただきました。
