...
例えば、fooプロダクトチームがApplicationで無認可のプロダクト用Cluster (例：`bar-cluster`) をデプロイ先として指定しようします。
...
例えば、fooプロダクトチームが、Applicationで無認可のNamespace (例：`app`) をデプロイ先に指定しようします。
...
Projectテナントは、application-controllerが無認可の空間 (Namespace、AppProject) 内にReconciliationすることを制限します。
