...
各プロダクトチームは、実Clusterテナントを介して、Applicationを操作します。
...
各プロダクトチームは、仮想Clusterテナントを介して、Applicationを操作します。
...
後述の [Projectテナント](#04-05-ツール固有テナント) は二重のテナントを持ち、Namespaceテナントも兼ねています。
...
Projectテナントは二重のテナント (**<font color="#FF0000">第一テナントにNamespace</font>**、**<font color="#FF0000">第二テナントに複数のAppProject</font>**) を持ち、両方のテナントで不許可なアクセスを防御します。
...
ArgoCDには、[Namespacedスコープモード](#05-NSモードなArgoCD---) と [Clusterスコープモード](#05-NSモードなArgoCD---) (以降、**"NSモード"** **"CLモード"**) があります。
