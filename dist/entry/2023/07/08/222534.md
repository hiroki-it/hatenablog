...
<table>
   ...
   <tbody>
      ...
      <tr>
         <td style="text-align: center;">各種ツールの独自テナント</td>
...
<table>
  <thead>
    ...
    <tr>
      <th><nobr>Namespacedスコープ</nobr><br>リソース<br>(ArgoCDの各コンポーネント、Application、AppProject)</th>
      <th><nobr>Clusterスコープ</nobr><br>リソース</th>
    </tr>
  </thead>
  <tbody style="text-align: center;">
    ...
    <tr>
      <td rowspan="2">Namespaces<br>as-a-Service</td>
      <td rowspan="2" >Namespace<br>テナント</td>
      <td>単一</td>
      <td><code>⭕️</code></td>
      <td></td>
      <td></td>
...
# 05. Control-planes as-a-Serviceの場合
...
# 06. Namespaces as-a-Serviceの場合

## Namespaceテナント (ArgoCDが単一)

### Namespaceテナント (ArgoCDが単一) とは

Namespaceテナント (ArgoCDが単一) では、テナントごとにNamespaceを作成しつつ、ArgoCDは共有します。
...
Namespaceテナント (ArgoCDが単一) には、以下のメリデメがあります。
...
Namespaceテナント (ArgoCDが単一) 間では、ClusterのKubernetesリソースを共有しますが、AppProject内のApplicationは共有しません。
...
## Namespaceテナント (ArgoCDが複数)

### Namespaceテナント (ArgoCDが複数) とは

Namespaceテナント (ArgoCDが複数) では、テナントごとにNamespaceとArgoCDを作成します。
