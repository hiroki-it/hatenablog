...
AppProjectに所属するApplicationがfooテナント以外にデプロイ先に指定できないようにしています。
...
認証済みのプロダクトチーム (foo-team、bar-team、baz-team) に対して、特定のAppProject (foo、bar、baz) に所属するApplicationのみを操作できるようにしています。
...
AppProjectに所属するApplicationが、fooテナント以外にデプロイ先に指定できないようにしています。
