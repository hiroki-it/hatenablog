...
プロダクトが稼働するClusterが`10`個以上あり、Clusterによっては複数のサブチームが`100`個以上のマイクロサービスを運用しています。

このような大規模なマイクロサービスシステムがいくつもある状況下で、ArgoCDのマルチテナント設計の知見を得られたため、それを記事で解説しました。
...
そのため、`--application-namespaces`オプションにアクセス可能なNamespaceを制限できます。
...
そのため、`.application.namespaces`キーでアクセス可能なNamespaceを制限する必要はありません。
...
<table>
  ...
  <tbody>
    <tr>
      <td style="text-align: center;"><a href="#パターン1">パターン1</a></td>
      <td style="text-align: center;">プロダクトの実行環境別</td>
      <td style="text-align: center;">プロダクトの実行環境別</td>
      <td style="text-align: center;"></td>
    </tr>
    <tr>
      <td style="text-align: center;"><a href="#パターン2--">パターン2</a></td>
      <td style="text-align: center;">プロダクト別</td>
      <td style="text-align: center;">プロダクトの実行環境別</td>
      <td style="text-align: center;">★</td>
    </tr>
    <tr>
      <td style="text-align: center;"><a href="#パターン3---">パターン3</a></td>
      <td style="text-align: center;">プロダクト別</td>
...
Projectテナントがマニフェストのデプロイをどのように制限するのかについて、エラーを挙げて解説します。

ここでは、前述の **["パターン3"](#パターン3---)** を採用したと仮定します。
...
Projectテナントは、以下のようなエラー🚫で、マニフェストのデプロイを制限します。
...
Projectテナントは、以下のようなエラー🚫で、ArgoCD系カスタムリソースのReconciliationを制限します。
