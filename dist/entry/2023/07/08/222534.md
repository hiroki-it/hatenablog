...
- ArgoCDのNamespacedスコープモード (NSモード) とClusterスコープモード (CLモード)
...
ControlPlanes as-a-Serviceなテナントの実践です。
...
ArgoCDの各コンポーネント (repo-server、application-controler) には、NamespacedスコープモードとClusterスコープモード (以降、"NSモード" "CLモード") があります。

| スコープモードの種類                | ArgoCDの影響範囲 | アクセス可能な範囲                                                          |
| ----------------------------------- | ---------------- | --------------------------------------------------------------------------- |
| Namespacedスコープモード (NSモード) | 分散             | ArgoCDの各コンポーネントは、自分自身が属するNamespace内のみにアクセスできる |
| Clusterスコープモード (CLモード)    | 中央集権         | ArgoCDの各コンポーネントは、許可された任意のNamespaceにアクセスできる       |
...
NSモードなArgoCDでは、各テナント間で独立したArgoCDを分散的に管理します。

また、ArgoCDの各コンポーネントは、自分自身が属するNamespace内のみにアクセスできます。
...
ただ、アップグレード後の新ArgoCD用のClusterを新しく構築してしまえばよいので、前述のデメリットはカバーできると判断しました。
...
CLモードなArgoCDでは、各テナント間で共有のArgoCDを中央集権的に管理します
...
NSモードと同様にして、`.spec.destination`キーで、テナント内のApplicationがいずれにデプロイ可能かを設定する必要があります。
...
# 05. NSモードなArgoCDのAppProjectテナントの分割パターン

## 概要

それでは、NSモードなArgoCDのAppProjectテナントをオススメする場合に、テナントをどのような粒度で作成するべきなのかを考えていきましょう。

前述の通り、NSモードなArgoCDでは、NamespaceとAppProjectの両方の粒度を考える必要があります。
