...
デメリットの解決策も考慮して、独断と偏見でオススメしませんでした。

| <nobr>アーキテクチャ</nobr><br>特性 | メリット ⭕️                                                                                                                                                    | デメリット ✖                                                                             |     | デメリットの解決策                                             |
| :---------------------------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | --- | -------------------------------------------------------------- |
|               拡張性                | -                                                                                                                                                              | テナントを増やすために実Clusterを作成する必要があり、作業量が多い。                       | ➡︎ | IaCツールで実Clusterを作成するようにすれば、作業量を減らせる。 |
|      安全性<br>(セキュリティ)       | 他のテナントからの通信を遮断できる。                                                                                                                           | -                                                                                         |     | -                                                              |
|               保守性                | 他のテナントとClusterスコープまたはNamespacedスコープなKubernetesリソースを分離できる。これらのKubernetesリソース (特にCRD) の変更が他のテナントに影響しない。 | 各テナントで実Clusterを継続的に保守しないといけない。(例：アップグレード、機能修正、など) | ➡︎ | 実体験として、妥協できないデメリット...😭                      |
|                性能                 | 他のテナントとClusterのハードウェアリソースを分け合わずに、これを独占できる。                                                                                  | -                                                                                         |     | -                                                              |
...
デメリットの解決策も考慮して、独断と偏見で **<font color="#FF0000">オススメ</font>** しました。
...
デメリットの解決策から、独断と偏見で **<font color="#FF0000">特にオススメ</font>** しました。
