...
今回は、KubernetesのマルチテナントパターンとArgoCDでのテナント設計の実践を記事で解説しました。
...
# 04. ArgoCDのテナント設計での実践

## 概要

Kubernetesでのマルチテナントパターンを解説しました。

ここからは、具体的にArgoCDのマルチテナント設計で候補になるパターンと、実際に適するパターン、を解説していきます。
...
`argocd-rbac-cm`は、認証済ユーザーやグループに、ArgoCD系カスタムリソース (Applicationなど) に関するパーミッションを紐づけます。
...
ArgoCDは`policy.csv`ファイルにCasbinの記法を採用しています。

| 記号                 | 説明                                               | 記法                                                                                                 |
| -------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `p` (パーミッション) | ロールとArgoCD系リソースの認可スコープを定義する。 | `p, <ロール名> <Kubernetesリソースの種類> <アクション名> <AppProject名>/<Namespace名>/<Kubernetesリソースの識別名>` |
