...
- NamespacedスコープモードなArgoCDが他のテナントへのアクセスを防ぐ仕組み
...
<table>
   ...
   <tbody>
      ...
      <tr>
         ...
         <td style="text-align: center;"><code>⭕️</code></td>
         <td style="text-align: center;"><code>⭕️</code></td>
         <td>仮想Cluster作成ツール (<a href="https://github.com/kcp-dev/kcp">Kcp</a>、<a href="https://github.com/virtual-kubelet/tensile-kube">tensile-kube</a>、<a href="https://github.com/loft-sh/vcluster">vcluster</a>、<a href="https://github.com/kubernetes-sigs/cluster-api-provider-nested/tree/main/virtualcluster">VirtualCluster</a>、など)</td>
      </tr>
      <tr>
      ...
      <tr>
         ...
         <td style="text-align: center;">ツールによる</td>
         <td><a href="https://argo-cd.readthedocs.io/en/stable/user-guide/projects/">ArgoCDのAppProject</a>、<a href="https://github.com/clastix/capsule">CapsuleのTenant</a>、<a href="https://github.com/loft-sh/kiosk">kioskのAccount</a>、<a href="https://github.com/kubewharf/kubezoo">KubeZooのTenant</a>、など</td>
      </tr>
   </tbody>
</table>
...
| :---------------------------------: | ----------------------------------------------------------------------------- |----------------------------------------------------------|
|               拡張性                | 他のテナントとClusterスコープなKubernetesリソース (特にCRD) を分離できる。    | テナントを増やすために、実際のClusterを作成する必要がある。                        |
...
テナントを増やす場合、コントロールプレーン (例：仮想Cluster) を新しく作成します。
...
例えばAppProjectテナントには、以下のメリデメがあります。
...
NSモードとの違いとして、`data.application.namespaces`キーで、argocd-serverとapplication-controllerがいずれのNamespaceにアクセスできるかを設定する必要があります。
...
今度は、NSモードなArgoCDのAppProjectテナントを採用する場合、おすすめのテナント例を解説していきます。
...
<table>
  ...
  <tr>
    ...
    <th colspan="2" style="text-align: center;">テナント例<br>(NamespaceとAppProjectの組み合わせ)</th>
...
また、`argocd-rbac-cm`というConfigMapを使用して、テナント内でArgoCD系カスタムリソースの認可スコープをさらに絞ることもできます。

これらを総合して、独断と偏見でオススメしました。

<br>

# 06. NSモードなArgoCDが他のテナントへのアクセスを防ぐ仕組み

そろそろ解説を読むのがしんどい方がいるのではないでしょうか。

『君がッ、泣くまで、解説をやめないッ!』

前述の **"高分離なテナント"** を例にして、簡単に仕組みを解説します。
...
{https://kubernetes.default.svc bar} is not permitted in project 'infra-team'
```
...
情報を詰め込みすぎた感があります (読者を置き去りにした パンッ!!)
