...
# 04-05. NamespacedスコープモードのAppProjectテナントの場合

## Namespacedスコープモードの場合

NamespacedスコープモードのAppProjectテナントを採用するためには、ArgoCDの『Namespacedスコープモード』を有効化する必要があります。

テナントごとにNamespaceとAppProjectを作成しつつ、各テナントで独立したArgoCDを管理します。
...
# 04-06. ClusterスコープモードのAppProjectテナントの場合

## Clusterスコープモードの場合

ClusterスコープモードのAppProjectテナントを採用するためには、ArgoCDの『Clusterスコープモード』を有効化する必要があります。

テナントごとにNamespaceとAppProjectを作成しつつ、各テナントで同じArgoCDを管理します。
