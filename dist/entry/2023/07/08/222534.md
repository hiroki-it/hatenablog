...
これに応じて、AppProjectテナントの設計方法が異なります。

| スコープモードの種類                   | ArgoCDの影響範囲 | ArgoCDのアクセス可能範囲                                                    |
| -------------------------------------- | ---------------- | --------------------------------------------------------------------------- |
| Namespacedスコープモード<br>(NSモード) | 各テナントで独立 | ArgoCDの各コンポーネントは、自分自身が属するNamespace内のみにアクセスできる |
...
# 05. NSモードなArgoCDのAppProjectテナントのテナント例

## オススメな設計の一覧

今度は、NSモードなArgoCDのAppProjectテナントを採用する場合に、おすすめのテナント例を解説していきます。
...
AppProjectテナントの全体像を整理しておきます。

<table>
  <thead>
    <tr>
      <th rowspan="2" style="text-align: center;">テナントの実装</th>
      <th rowspan="2" style="text-align: center;">スコープ<br>モード</th>
      <th colspan="2" style="text-align: center;"><nobr>AppProject<br>テナント例</nobr></th>
      <th rowspan="2" style="text-align: center;"><nobr>オススメ</nobr></th>
    </tr>
  <tr>
    <th>Namespace例</th>
  ...
  <tbody style="text-align: center;">
    <tr>
      <td rowspan="4">AppProject</td>
      <td rowspan="3">Namespaced<br>スコープモード</td>
      <td>実行環境別</td>
      <td>実行環境別</td>
      <td></td>
    </tr>
