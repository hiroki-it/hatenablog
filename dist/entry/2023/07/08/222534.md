...
# 05-02. 高分離なAppProjectテナントが他のテナントへのアクセスを防ぐ仕組み
...
認可エラー例を挙げて、AppProjectが他のテナントへのアクセスを防ぐ仕組みを簡単に解説します。

全ての認可エラーはArgoCDの単体テストから確認できますので、興味のあるかたはぜひ🙇🏻‍
...
### (認可エラー例2) 無許可のNamespaceでApplication作成
...
### (認可エラー例3) 無許可のNamespaceでApplication作成

もし、`foo`プロダクトチームが無許可のNamespace (例：`app`) にマニフェストをデプロイしようとすると、argocd-serverは`destination`の認可エラーを返却します。
