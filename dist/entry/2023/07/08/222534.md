...
## Namespacedスコープモードをおすすめした理由

NamespacedスコープモードArgoCDのAppProjectテナント間では、ClusterスコープなKubernetesリソースは共有してしまいますが、ArgoCDの各コンポーネントを共有します。
...
ただ、ArgoCDのアップグレード時はCluster自体をブルー/グリーン方式でアップグレードすればよいため、このデメリットは "根性" でカバーできます。

これらを総合して、独断と偏見でおすすめしました。

<br>

## ClusterスコープモードArgoCDの場合
...
Namespacedスコープモードと同様にして、`.spec.destination`キーで、テナント内のApplicationがいずれにデプロイ可能かを設定する必要があります。
...
## Clusterスコープモードをおすすめしなかった理由

ClusterスコープモードArgoCDのAppProjectテナント間では、ClusterスコープなKubernetesリソースは共有してしまいますが、ArgoCDの各コンポーネントを共有します。
...
# 05. AppProjectテナントの分割パターン

## 概要

それでは、AppProjectテナントをおすすめする場合に、テナントをどのような粒度で作成するべきなのかを考えていきましょう。

前述の通り、AppProjectはNamespacedスコープなカスタムリソースなため、NamespaceとAppProjectの両方の粒度を考える必要があります。
