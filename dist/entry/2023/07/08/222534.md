...
前述の **"高分離なAppProjectテナント"** を例にして、簡単に仕組みを解説します。
...
#### ▼ (エラー例1) 無許可のNamespaceでApplication作成

もし、`foo`プロダクトチームが無許可のNamespace (例：`bar`) でApplicationを作成しようとすると、argocd-serverは`destination`のエラーを返却します
...
もし、`foo`プロダクトチームがApplicationで無許可のCluster (例：`bar-cluster`) をデプロイ先として指定しようとすると、argocd-serverは`destination`のエラーを返却します。
...
#### ▼ (エラー例3) 無許可のNamespaceでApplication作成

もし、`foo`プロダクトチームが無許可のNamespace (例：`app`) にマニフェストをデプロイしようとすると、argocd-serverは`destination`のエラーを返却します。
