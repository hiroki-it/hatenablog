...
- [AppProjectテナントの例](<#05-AppProjectテナント例-(NSモードの場合)>) (5章)
...
<table>
  ...
  <tbody style="text-align: center;">
    <tr>
      <td><a href="#Clusters-as-a-Service">Clusters<br>as-a-Service</a></td>
      <td>実Cluster</td>
      <td>独立</td>
      <td>✅</td>
      <td>✅</td>
...
ただ、後述の **<font color="#FF0000">AppProjectテナントは、Namespaceテナントを兼ねるパターン</font>** であり、二層テナントを設計できます。
...
そのため、**<font color="#FF0000">AppProjectテナントはNamespaceテナントを兼ねる二層なテナントパターン</font>** です。
...
# 05. AppProjectテナント例 (NSモードの場合)

## AppProjectテナント実践の一覧

今度は、NSモードなArgoCDのAppProjectテナントを採用する場合、おすすめのAppProject例を解説していきます。

AppProjectテナントが、NamespaceとAppProjectの二層テナントになっていることにも、留意してください。
...
<div class="text-box">
本記事では言及しませんが、Namespaceにもいくつかの分割パターンがあり、これも取り入れてAppProjectテナントをテナントパターンすると良いです。
<br>
<br>
...
| <nobr>アーキテクチャ</nobr><br>特性 | メリット ⭕️                                                                                                             | デメリット **×**                                                                                                          |
| :---------------------------------: | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
|               拡張性                | -                                                                                                                       | ArgoCD数が多くなるため、Nodeの上限Pod数やIPアドレスの使用可能数にひっかかり、それ以上AppProjectテナントの増やせなくなる。 |
...
AppProjectテナントは、ログインユーザーが無許可なテナントのマニフェストを操作することを防ぎます。
