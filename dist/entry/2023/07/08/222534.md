...
予告となりますが、**次回の記事ではArgoCDがテナント間でどのように操作を制限するのかの仕組み**を解説するゾ〜
...
これらのスコープモードによって、NamespaceやAppPro
...
metadata:
  ...
  namespace: foo
spec:
  # このAppProjectへの所属を許可したいApplicationのNamespaceを設定する
  sourceNamespaces:
    - foo-application-ns
  # 所属しているApplicationを操作して、いずれにデプロイ可能かを設定する
  destinations:
...
metadata:
  ...
  namespace: foo
data:
  # application-controllerとargocd-serverがいずれのNamespaceにアクセスできるかを設定する
...
次回は、**ArgoCDがテナント間でどのように操作を制限するのかの仕組み**を解説するゾイ🐧
