...
`.spec.destination`キー配下に、テナント内のApplicationがいずれにデプロイ可能かを設定する必要があります。

Namespacedスコープモードとの大きな違いとして、`.spec.sourceNamespaces`キーの設定が必要です。
...
application-controllerとargocd-serverがいずれのNamespaceにアクセスできるかを設定する必要があります。
...
argocd-cmd-params-cmではなく、application-controllerの起動時にパラメーターとして渡しても良いです。
