...
後述のProjectテナントは、二重のテナント境界線を持っており、Namespaceテナントも兼ねています。
...
Projectテナントでは、テナントごとにNamespaceと複数のAppProjectを作成し、テナントで使用する **<font color="#FF0000">Applicationをグループ化</font>** します。

AppProjectのCRDの`.spec.scope`キーからも分かる通り、AppProjectはNamespacedスコープなカスタムリソースです。

そのため、Projectテナントは二重のテナント (**<font color="#FF0000">第一テナントにNamespace、第二テナントに複数のAppProject</font>**) を持っています。
...
この場合に、プロダクトの実行環境別にNamespace (第一テナント) とAppProject (第二テナント) を作成します。
...
プロダクト別にNamespace (第一テナント) 、プロダクトの実行環境別にAppProject (第二テナント) を作成します。
...
プロダクト別にNamespace (第一テナント) 、プロダクトのサブチーム別にAppProject (第二テナント) を作成します。
