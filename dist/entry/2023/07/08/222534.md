...
また、ArgoCDのデプロイ対象のプロダクトが複数おり、プロダクトの実行環境 (例：Tes-1環境、Tes-2環境) 別にClusterがいるとします。
...
同様にして、プロダクトごとに異なる管理チームがおり、プロダクトの実行環境 (例：Tes-1環境、Tes-2環境) 別にClusterがいると仮定します。
...
各プロダクトの管理チームがダッシュボード (argocd-server) にSSOを使用してログインしようとします。
