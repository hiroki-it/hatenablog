...
- ArgoCDのNamespacedスコープモードとClusterスコープモードについて
- NamespacedスコープモードArgoCDの場合の各テナント内での認可処理の仕組み
...
マルチテナントの概念から実践までを記載したところ、文量がエグいことになってしまったので、雰囲気だけ拾って帰っていただけると嬉しいです🙏
...
# 08-02. NamespacedスコープモードArgoCD

## AppProjectテナント

NamespacedスコープモードArgoCDでは、各テナントで独立したArgoCDを管理しつつ、テナントごとにNamespaceとAppProjectを作成します。
...
# 08-03. ClusterスコープモードArgoCD

## AppProjectテナント

ClusterスコープモードArgoCDでは、各テナントで同じArgoCDを管理しつつ、テナントごとにNamespaceとAppProjectを作成します。
