...
- NamespacedスコープモードなArgoCDが無許可のテナントへのアクセスを防ぐ仕組み
...
# 05-02. 高分離なAppProjectテナントが無許可のテナントへのアクセスを防ぐ仕組み
...
認可エラー例をいくつか挙げて、AppProjectテナントが無許可のテナント (他のテナントのNamespace、AppProject、デプロイ先Cluster) への様々なアクセスを防ぐ仕組みを簡単に解説します。

なお、全ての認可エラーはArgoCDの単体テストから確認できますので、興味のあるかたはぜひ🙇🏻‍

> - https://github.com/argoproj/argo-cd/blob/v2.7.10/util/argo/argo_test.go
...
AppProjectテナントは、argocd-serverが無許可のテナントでマニフェストを操作することを防ぎます。
...
### (認可エラー例1) 無許可のNamespaceでApplication作成

もし、`foo`プロダクトチームが無許可のNamespace (例：`bar`) でApplicationを作成しようとすると、argocd-serverは`destination`の認可エラーを返却します

仮に無許可のNamespaceでApplicationを作れてしまうと、そのApplicationから、誤ったClusterにマニフェストをデプロイできてしまいます😈
...
### (認可エラー例2) 無許可のClusterをデプロイ先に指定

もし、`foo`プロダクトチームがApplicationで無許可のCluster (例：`bar-cluster`) をデプロイ先として指定しようとすると、argocd-serverは`destination`の認可エラーを返却します。

仮に無許可のClusterをデプロイ先に指定できてしまうと、誤ったClusterにマニフェストをデプロイできてしまいます😈
...
### (認可エラー例3) 無許可のNamespaceをデプロイ先に指定

もし、`foo`プロダクトチームが無許可のNamespace (例：`app`) をデプロイ先に指定しようとすると、argocd-serverは`destination`の認可エラーを返却します。

仮に無許可のNamespaceをデプロイ先に指定できてしまうと、誤ったNamespaceにマニフェストをデプロイできてしまいます😈
...
AppProjectテナントは、application-controllerが無許可のテナントにReconciliationすることを防ぎます。
