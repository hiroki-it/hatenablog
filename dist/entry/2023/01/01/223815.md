...
Istioのサイドカーによるサービスメッシュ（サイドカーメッシュ）は、サイドカーコンテナ（```istio-proxy```コンテナ）が稼働するデータプレーン、サイドカーを中央集権的に管理するIstiod（```discovery```コンテナ）が稼働するコントロールプレーン、からなります。
...
本題と話が逸れるため今回は詳しく言及しませんが、kube-apiserverの仕組みについては、以下の記事が参考になると思います🙇🏻‍♂️
