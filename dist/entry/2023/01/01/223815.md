...
- [04. サイドカーコンテナのインジェクション](#04-サイドカーコンテナのインジェクション)
- [05. おわりに](#05-おわりに)
...
執筆時点（2023/01/05）では、Istioが実装するサービメッシュには、『サイドカーメッシュ』と『アンビエントメッシュ』があります。
...
一つの方法として、アプリエンジニアが各マイクロサービス内にインフラ領域の問題に関するロジックを実装すれば、それらを解決することができます。
...
また、kube-apiserver、Service、discoveryコンテナ内のwebhookサーバー、に着目すると以下のような仕組みになっています。
...
今回は詳しく言及しませんが、この時にPodが作成されるわけではありません。
...
本題と逸れるため説明を省略しますが、もし知りたい方は以下の記事が参考になると思います🙇🏻‍♂️
