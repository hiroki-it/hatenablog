...
クライアントが、Pod（Deployment、DaemonSet、StatefulSet、も含む）の作成リクエストをkube-apiserverに送信します。
...
### （５）webhookサーバーにリクエストを送信

kube-apiserverは、mutating-admissionステップにて、webhookサーバーにAdmissionReview構造体を持つリクエストを送信します。
...
Istioでは、webhookサーバーは```discovery```コンテナ内でhttpサーバー処理として実行されています。
...
前段にあるServiceはリクエストを受信し、```discovery```コンテナの```15017```番ポートにポートフォワーディングします。
...
本記事とは話が逸れるため、今回は詳しく言及しませんが、上記のpathc処理では、```istio-proxy```コンテナの他に、```istio-init```コンテナもインジェクションできるようにします。
