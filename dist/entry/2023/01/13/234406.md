...
MutatingAdmissionWebhookプラグインの発火条件に関して、例えばIstioでは、[NamespaceやPodの```metadata.labels```キーに応じてサイドカーインジェクションの有効化/無効化を切り替えることができ](https://istio.io/latest/docs/setup/additional-setup/sidecar-injection/#controlling-the-injection-policy)、これをMutatingAdmissionWebhookプラグインで制御しています。
