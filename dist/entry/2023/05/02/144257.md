...
さて最近の業務で、全プロダクトが共通基盤として使用するArgoCD🐙とAWS EKS Clusterのアーキテクチャをリプレイスしています。
...
<div class="text-box">
Kustomizeプラグインによるマニフェスト作成は、サイドカーではなくrepo-serverで実行した方が良いかもしれません (Helmプラグインはサイドカーで問題ないです)。
<br>
<br>
...
<br>
<br>
無理やりサイドカーでKustomizeプラグインを実行しようとすると、ArgoCDの既存のオプションを無視した実装になってしまうため、Kustomizeプラグインだけはrepo-serverで実行することをお勧めします😢
</div>
...
逆を言えば、**<font color="#FF0000">プラグイン名を指定していないApplicationは、サイドカーをコールせずに、repo-serverにデフォルトでインストールされているマニフェスト管理ツール (例：Helm、Kustomize) を使用してマニフェストを作成します。</font>**
...
<div class="text-box">
プラグインを必要としないApplicationは、cmp-serverをコールしません。
<br>
<br>
ArgoCDのコンテナイメージ (<code>quay.io/argoproj/argocd</code>) には、いくつかのツール (例：Helm、Kustomize、Ks、Jsonnet、など) の推奨バージョンがあらかじめインストールされています。
<br>
<br>
そのため上記のApplicationは、repo-server上にあるツールを使用して、マニフェストを作成します。
<br>
<br>
cmp-server上のツールは、repo-server内から持ってくれば良いのでは？と思った方がいるかもしれません。
<br>
<br>
ただ、repo-server内からツールを無理やりcmp-serverに移動させるのは実装が複雑になるため、個人的にはcmp-serverのためにツールをインストールした方が良いです😭
<br>
<br>
