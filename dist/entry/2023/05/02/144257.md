...
```bash
$ curl -s https://raw.githubusercontent.com/argoproj/argo-cd/<タグ>/hack/tool-versions.sh | grep version=
```

<br>
<blockquote>
↪️：[https://github.com/argoproj/argo-cd/blob/main/hack/tool-versions.sh:title]
<br>
<br>
</blockquote>
</div>
...
<div class="text-box">
デプロイ先Cluster管理者の認可スコープだけでなく、application-controllerとargocd-serverにも認可スコープが必要な場合があります。
<br>
<br>
今回の図のように、単一のArgoCD用Clusterで複数プロダクトのApplicationを管理し、Applicationをプロダクト別のAppProjectで分割している場合です。
<br>
<br>
application-controllerとargocd-serverが、対応するプロダクトのApplicationのみを操作できるように制限するためには、ConfigMap (argocd-cmd-params-cm) とAppProjectで設定が必要です👍
<br>
<br>
前提として、fooプロダクトのApplicationはfooというNamespaceにいるとします。
...
metadata:
  ...
  namespace: argocd
data:
  application.namespaces: "foo" # fooというNamespaceのみを許可する。
```
