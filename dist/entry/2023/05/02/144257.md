...
<div class="text-box">
デプロイ先Cluster管理者が、いずれのNamespaceでApplicationを作成できるのかの認可スコープが必要な場合があります。
<br>
<br>
今回の図のように、単一のArgoCD用Clusterで複数プロダクトのApplicationを管理し、AppProjectでテナントを分割している場合です。
<br>
<br>
デプロイ先Cluster管理者が、対応するプロダクトのApplicationのみを作成できるように制限するためには、ConfigMap (argocd-cmd-params-cm) とAppProjectで設定が必要です👍
<br>
<br>
...
metadata:
  ...
  namespace: foo
spec:
  # foo-projectに属するArgoCD系リソースをいずれのNamespaceで作成できるか
  sourceNamespaces:
    - "foo" # fooというNamespaceのみでArgoCD系リソースを作成することを許可する。
```

ArgoCDのApplication (<code>foo-application</code>) は、<code>foo-project</code>に属しているため、<code>foo</code>というNamespaceにしか作成できません。
