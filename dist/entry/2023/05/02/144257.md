...
data:
  ...
  url: https://github.com/hiroki-hasegawa/foo-chart.git
---
...
<div class="text-box">
今回の図のように、単一のArgoCD用Clusterで複数プロダクトのApplicationを管理する場合、AppProjectを単位としたテナント分割を設定した方が良いです。
<br>
<br>
その場合、ArgoCD本体 (特に、application-controllerとargocd-server) とデプロイ先Clusterの両方の認可スコープを考える必要があります。
<br>
<br>
ArgoCD本体には、Cluster全体への認可スコープを与えます。
...
metadata:
  ...
  namespace: argocd
data:
  # application-controllerとargocd-serverは全てのNamespaceにアクセスできる
  application.namespaces: "*" # 全てのNamespaceを許可する。
```

デプロイ先Cluster管理者には、AppProjectを単位とした認可スコープを与えます。
...
spec:
  sourceNamespaces:
    - "foo" # dev-foo-projectにアクセス可能なデプロイ先Cluster管理者は、fooというNamespaceのみでArgoCD系リソースを作成できる。
```
...
data:
  ...
  policy.default: role:developer
  policy.csv: |
    # foo-developerは、dev-foo-projectに属するArgoCD系リソースのみ操作できる
    p, role:foo-developer, *, *, dev-foo-project/*, allow
    
    # foo-maintainerは、dev-foo-projectとstg-foo-projectの両方を操作できる
    p, role:foo-maintainer, *, *, dev-foo-project/*, allow
    p, role:foo-maintainer, *, *, stg-foo-project/*, allow
...
これらにより、デプロイ先Cluster管理者は担当していないプロダクトのArgoCD系リソースにはアクセスできなくなり、当然デプロイもできなくなります👍

<blockquote>
↪️：[https://argo-cd.readthedocs.io/en/stable/operator-manual/app-any-namespace/#implementation-details:title]
<br>
<br>
</blockquote>
</div>
