...
認可レスポンスは、argocd-serverを介して、再びdex-serverに届きます。

この後、dex-serverはIDプロバイダーのトークンエンドポイントにリクエストを送信し、またIDプロバイダーからユーザー情報やトークンを取得します。

これ以降はSSOの仕組みの解説になってしまうため、詳細は省略します。
