...
<br>
<br>
これは、<code>127.0.0.1</code>を指定してしまうと、iptables上で通信がループしてしまうためです。
<br>
<br>
正しくは`ISTIO_OUTPUT`から`POSTROUTING`にパケットが渡るところを、`ISTIO_OUTPUT`から`ISTIO_IN_REDIRECT`に渡ってしまい、これがループになっているためです😭
<br>
<blockquote>
<ul>
