...
送信元または宛先のEnvoyに分けると、各リソースは以下のようにEnvoyを抽象化します。

両方のEnvoyは同じリソースで抽象化しており、送信元のEnvoyで設定を紐づけていく処理を宛先のEnvoyでも同様に実施します。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align: center;">Virtual<br>Service<br>Y</td>
      <td style="text-align: center;">Destination<br>Rule<br>X</td>
      <td style="text-align: center;">Destination<br>Rule<br>Y</td>
      <td style="text-align: center;">Service<br>Entry</td>
      <td style="text-align: center;">Peer<br>Authentication</td>
    </tr>
    <tr>
...
<ul class="decimal">
  ...
  <li>HTTPフィルターにより、リクエストの宛先ホスト (例：<code>example.com</code>) やパス (例：<code>/</code>) で、Serviceを絞り込みます。Envoyは、クラスターを宛先ポートやホストで管理しています (例：<code>inbound|50010|foo-service.foo-namespace.svc.cluster.local</code>) </li>
