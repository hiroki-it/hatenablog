...
<div class="text-box">
<div class="text-box-title">▶︎ 宛先Pod内のiptablesについて</div>
<br>
宛先Pod内のiptablesは、リクエストが必ず<code>istio-proxy</code>コンテナを経由するように、<code>istio-proxy</code>コンテナにリクエストをリダイレクトします。
<br>
<br>
iptablesのルールを書き換えるのは<code>istio-init</code>コンテナで、Istioは<code>istio-proxy</code>コンテナと同じタイミングで<code>istio-init</code>コンテナをPodにインジェクションします。
<br>
<br>
<img src="https://raw.githubusercontent.com/hiroki-it/tech-notebook-images/master/images/drawio/blog/istio/istio_istio-init.png" alt="istio_istio-init" style="zoom:60%;">
<br>
<br>
<code>istio-init</code>コンテナは、<code>istio-iptables</code>コマンドを実行し、iptablesのルールを書き換えます。
<br>
<br>
また、コンテナ自体は書き換えた後に終了するため、Pod内には残りません👍🏻
...
<blockquote>
<ul>
<li>[https://www.sobyte.net/post/2022-07/istio-sidecar-proxy/#sidecar-traffic-interception-basic-process:title]</li>
...
<div class="text-box">
<div class="text-box-title">▶︎ Istio IngressGateway内のiptablesについて</div>
<br>
Istio IngressGateway Podでは、iptablesでリダイレクトせずとも<code>istio-proxy</code>コンテナにリクエストを送信できます。
<br>
<br>
Istioは、iptablesのルールを書き換える<code>istio-init</code>コンテナをIstio IngressGateway Podにインジェクションしないため、iptablesのルールはデフォルトのままになっています👍🏻
</div>
