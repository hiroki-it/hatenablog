...
<table>
  <thead>
    <tr>
      <th style="text-align: center;">フィルターの種類</th>
      <th style="text-align: center;"><nobr>HTTPSを処理するフィルター</nobr><br>(一部抜粋)</th>
  ...
  <tbody>
    ...
    <tr>
      <td>TLS Inspector</td>
      <td>EnvoyがTLSを検知できるようにします。<br>通信にTLSを使用している場合、Envoyはリスナーフィルターの後にTLS Transport socketを実行します。<br>TLS Transport socketは、TLSハンドシェイクを実行し、パケットのL7ヘッダーを復号化します。
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>Router</td>
      <td>Envoyが、ポート番号でルート、ホストやパスでクラスターを絞り込めるようにする。</td>
    </tr>
    <tr>
      <td>gRPC-Web</td>
      <td>Envoyが、HTTP/1.1で受信したリクエストをgRPCサーバーをプロキシできるようにする。</td>
    </tr>
  </tbody>
</table>
