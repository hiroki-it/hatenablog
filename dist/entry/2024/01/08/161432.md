---
Title: 【Istio⛵️】IstioのサイドカーメッシュによるEnvoy抽象化の仕組み
...
- KubernetesリソースやIstioカスタムリソースに基づくEnvoy抽象化の仕組みについて
- Istioサイドカーメッシュ/アンビエントメッシュ、Consul、Ciliumサイドカーフリーメッシュ、などで汎用的に活かせるEnvoyの仕組みについて
...
これは、Istioサイドカーメッシュ/アンビエントメッシュ、Consul、Istioから得られた学びを土台に登場したCiliumサイドカーフリーメッシュ、なども同様です。
...
つまり、次々に登場するサービスメッシュツールに振り回されないようにするためには、Envoyの設定の抽象化の仕組みを理解しておく必要があります。

そこで今回は、Istioサイドカーメッシュを題材にして、IstioがEnvoyのトラフィック管理の設定をどのように抽象化するのかを解説します。

もちろん、Envoy自体も理解しておく必要があるため、Envoyの仕組みも一緒に解説します。
...
Istioサイドカーメッシュを題材にして、IstioがEnvoyのトラフィック管理の設定をどのように抽象化するのかを解説しました。

次々とサービスメッシュツールが登場したとしても、Envoyを使用したサービスメッシュである限り、最終的にはEnvoyの設定値に行き着きます。

そのため、Envoy抽象化の仕組みを理解すれば、様々なサービスメッシュツールで知識を流用できると思います。

Istioサイドカーメッシュはもちろん、IstioアンビエントメッシュやEnvoyによる他のサービスメッシュツールを使っている方の参考にもなれば幸いです👍🏻
