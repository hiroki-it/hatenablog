---
Title: 【Istio⛵️】Istioによって抽象化されるEnvoyのHTTPS処理の仕組み
Category:
  - Istio
  - Envoy
  - Kubernetes
...
つまり、次々に登場するEnvoyによるサービスメッシュツールに振り回されないようにするためには、Istioによって抽象化されるEnvoyのHTTPS処理の仕組みを理解しておく必要があります。

そこで今回は、Istioサイドカーメッシュを題材にして、IstioがEnvoyのHTTPS処理をどのように抽象化するのかを解説します。
...
まずは、どのようなリソースがHTTPSの処理に関係しているのかを、リクエストの方向に分けて解説していきます。
...
この`istio-proxy`コンテナを使用してどのようにHTTPSを処理しているのかを、リクエストの方向に分けて解説します。
...
Istioサイドカーメッシュを題材にして、IstioがEnvoyのHTTPS処理をどのように抽象化するのか、またEnvoyの仕組みを解説しました。
