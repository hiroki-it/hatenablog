...
また、抽象化されたEnvoyがHTTPSを処理する仕組みも一緒に解説します。
...
まずは、どのようなリソースがHTTPSリクエストの処理に関係しているのかを、リクエストの方向に分けて解説していきます。
...
1. クライアントは、サービスメッシュ外からL7ロードバランサーにリクエストを送信します。
...
2. クライアントは、サービスメッシュ外からL7ロードバランサーにリクエストを送信します。
...
- HTTPSリクエストの処理に関係するネットワークフィルターやHTTPフィルター
