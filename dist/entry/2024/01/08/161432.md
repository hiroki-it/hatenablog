...
この`istio-proxy`コンテナを使用してどのようにHTTPSを処理しているのかを、HTTPSリクエストの方向に分けて解説します。
...
3. Config servingレイヤーにて、Envoyの設定をPod内の`istio-proxy`コンテナに配布します。双方向ストリーミングRPCのため、PodがEnvoyの設定配布レイヤーにHTTPSリクエストを送信し、これらを取得することもあります。
...
HTTPSを処理する場合、Envoyの設定が以下の順で紐付き、HTTPSリクエストを送信元から宛先まで届けます。
...
1. 送信元マイクロサービスからのHTTPSリクエストの宛先ポートで、リスナーを絞り込みます。
...
HTTPSのHTTPSリクエストを処理する場合、リスナーフィルター、ネットワークフィルター、HTTPフィルター、に属するフィルターを使用します。
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>istio-proxy</code>コンテナへのリダイレクト前の宛先情報をEnvoyが取得できるようにします。<br>iptablesがHTTPSリクエストを<code>istio-proxy</code>コンテナにリダイレクトすると、HTTPSリクエストの宛先が<code>istio-proxy</code>コンテナに変わってしまいます。<br>ただし、iptablesはリダイレクト前の宛先を<code>SO_ORIGINAL_DST</code>という定数に格納してくれています。<br>Envoyは<code>SO_ORIGINAL_DST</code>からこれを取得し、本来の宛先にプロキシします。</td>
    </tr>
    <tr>
...
6. パケットのL7ヘッダーを暗号化した上で、HTTPSリクエストを宛先PodにL7ロードバランシングします。
...
- Istioのトラフィック管理におけるHTTPSリクエスト方向の種類
