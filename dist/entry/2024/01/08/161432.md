...
前章では、`istio-proxy`コンテナ内のEnvoy設定まで、言及しませんでした。

本章では、もっと具体化します。

いよいよ、Istioが各リソースをいずれのEnvoy設定に翻訳しているのかを解説します。
...
1. リスナーフィルターを実行します。
   1. Envoyはiptablesがリクエストをリダイレクトする前の宛先を`SO_ORIGINAL_DST`という定数から取得します。
   2. TLSで暗号化された通信 (例：HTTPS) であるかを確認します。
2. TLSで暗号化された通信であった場合、TLSハンドシェイクを実行し、パケットのL7ヘッダーを復号化します。フィルター後のL7ロードバランシング時に、L7ヘッダーを再び暗号化します。
3. ネットワークフィルターを実行します。
   1. L3/L4ヘッダーを読み取り/書き込みます。
4. HTTPフィルターを実行します。
   1. ポート番号を使用して、後続のルート値を絞りこみます。
   2. ホストやパスを使用して、後続のクラスター値を絞りこみます。

<div class="text-box">
iptablesがリクエストをリダイレクトすると、送信元マイクロサービスの宛先が本来のものから`istio-proxy`コンテナに変わってしまいます。
<br>
<br>
しかし、iptablesは`SO_ORIGINAL_DST`という定数に本来の宛先を格納した上で、リダイレクトしてくれます。
<br>
<br>
そのため、Envoyは`SO_ORIGINAL_DST`から本来の宛先を取得し、本来の宛先に戻した上で、宛先にリクエストをプロキシできるというわけです👍🏻
</div>
...
Istioコントロールプレーンは、KubernetesリソースやIstioカスタムリソースの設定をEnvoy設定に翻訳し、処理の流れに適用します。

以下の通り、各リソースがいずれのEnvoy設定の抽象化に関わるのかを整理しました。
...
<tbody>
    <tr>
      <td style="text-align: center;"><nobr>Envoy設定</nobr></td>
...
前述の表を参考に、各リソースとEnvoy設定の関係を実際の処理の流れに適用します。
...
リソースがEnvoy設定間で重複していてわかりにくいので、少し簡略化します。
...
前章では、Envoy設定に関わる各リソースの設定まで、言及しませんでした。

本章では、さらに具体化します。

各リソースの設定の翻訳によって、Envoyの設定がどのようになっているのかを解説します。

なお、以下のコマンドを実行すると、`istio-proxy`コンテナのEnvoy設定を確認できます👍
