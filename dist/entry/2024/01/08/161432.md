...
1. リクエストの宛先ポートで、リスナー値を絞り込みます。
2. 通信の種類 (例：HTTP、HTTPS、TCP、UDP、など) に応じてフィルターを選び、各フィルターがパケットのヘッダーを処理します。
3. リクエストの宛先ポートで、ルート値を絞り込みます。
4. リクエストの宛先ホスト/パスで、クラスター値を絞り込みます。
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、クラスター配下のエンドポイント値を選びます。
...
リスナーフィルター、ネットワークフィルター、HTTPフィルター (HTTP/HTTPSを処理する場合のみ) に属するいくつかのフィルターが必要であり、これらはデフォルトで有効になっています (ここでは個別のフィルター名は省略します)。

図中の番号に沿って、仕組みを解説します。

1. リスナーフィルターを実行します。
   - Envoyはiptablesがリクエストをリダイレクトする前の宛先を`SO_ORIGINAL_DST`という定数から取得します。
   - TLSで暗号化された通信 (例：HTTPS) であるかを確認します。
2. TLSで暗号化された通信であった場合、TLSハンドシェイクを実行し、パケットのL7ヘッダーを復号化します。フィルター後のL7ロードバランシング時に、L7ヘッダーを再び暗号化します。
3. ネットワークフィルターを実行します。
   - L3/L4ヘッダーを読み取り/書き込みます。
4. HTTPフィルターを実行します。なお、あくまでHTTPフィルターはHTTP/HTTPSを処理する場合にのみ使用します。それ以外の通信 (例：TCP) の場合は、HTTPフィルターを使用しません。
   - ポート番号を使用して、後続のルート値を絞りこみます。
   - ホストやパスを使用して、後続のクラスター値を絞りこみます。
...
- フィルターを抽象化するリソースはEnvoyフィルターですが、フィルターのデフォルト値でも問題なく使えるため、EnvoyFilterは省略します。
