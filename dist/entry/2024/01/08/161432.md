...
3. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。
4. HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、Service配下のPodを選びます。
6. パケットのL7ヘッダーを暗号化した上で、リクエストを宛先PodにL7ロードバランシングします。

### 宛先

7. L7ロードバランシングされたリクエストの宛先ポートで、リスナーを絞り込みます。
8. HTTPSを処理するためのフィルターを選びます。
9. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。
10. HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。
11. Service配下のPodとして、ローカルホストを選びます
...
2. HTTPSを処理するためのフィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。
4. HTTPフィルターにより、Istio EgressGatewayに宛先を変更します。
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、Istio EgressGateway Service配下のPodを選びます。
