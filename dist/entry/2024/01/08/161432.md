...
リソースによっては、Envoyの異なる設定に渡って、これを抽象化します。
...
前述の表を参考に、各リソースとEnvoyの設定の関係を実際の処理の流れに当てはめます。
...
<table>
  <thead>
    <tr>
      <th colspan="2"></th>
      <th colspan="2">Kubernetes ☸️<br>リソース </th>
      <th colspan="4">Istio ⛵️<br>カスタムリソース </th>
    </tr>
  </thead>
  <tbody>
    <tr>
    ...
    <tr>
      <td rowspan="4" style="text-align: center; background-color: #CCFFFF">送信元<br><code>istio-proxy</code></td>
    ...
    <tr>
      <td rowspan="4" style="text-align: center; background-color: #FFFF88">宛先<br><code>istio-proxy</code></td>
...
<table>
  <thead>
    <tr>
      <th colspan="2"></th>
      <th colspan="2">Kubernetes ☸️<br>リソース </th>
      <th colspan="3">Istio ⛵️<br>カスタムリソース </th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
<table>
  <thead>
    <tr>
      <th colspan="2"></th>
      <th colspan="2">Kubernetes ☸️<br>リソース </th>
      <th colspan="5">Istio ⛵️<br>カスタムリソース </th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
1. 送信元マイクロサービスのリクエストの宛先ポート (例：`50000`) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで管理しています (例：`0.0.0.0_50000`) 。
2. HTTPSを処理するための各種フィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポート (例：`50000`) で、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで管理しています (例：`50000`) 。
4. HTTPフィルターにより、リクエストの宛先ホスト (例：`foo-service.foo-namespace.svc.cluster.local`) やパス (例：`/`) で、Serviceを絞り込みます。Envoyは、クラスターを宛先ポートやホストで管理しています (例：`outbound|50010|foo-service.foo-namespace.svc.cluster.local`) 。
...
<ul class="decimal">
  ...
  <li>HTTPフィルターにより、リクエストの宛先ポート (例：<code>50000</code>) で、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで管理しています (例：<code>50000</code>) 。</li>
  <li>HTTPフィルターにより、リクエストの宛先ホスト (例：<code>example.com</code>) やパス (例：<code>/</code>) で、Serviceを絞り込みます。これが、クラスター (例：<code>inbound|50000|foo-service.foo-namespace.svc.cluster.local</code>) になります。</li>
  <li>エンドポイントを選びます。Envoyは、エンドポイントをローカルホストや宛先ポートで管理しています (例：<code>127.0.0.1:50000</code>) 。
  <li>ローカルホストにリクエストを送信します。結果的に、宛先マイクロサービスに通信が届きます。</li>
</ul>
...
1. 送信元マイクロサービスのリクエストの宛先ポート (例：`50010`) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで管理しています (例：`0.0.0.0_50010`) 。
2. HTTPSを処理するための各種フィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポート (例：`50010`) で、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで管理しています (例：`50010`) 。
4. HTTPフィルターにより、リクエストの宛先ホスト (例：`foo-service.foo-namespace.svc.cluster.local`) やパス (例：`/`) で、Serviceを絞り込みます。Envoyは、クラスターを宛先ポートやホストで管理しています (例：`outbound|50010|foo-service.foo-namespace.svc.cluster.local`) 。
...
<ul class="decimal">
  ...
  <li>HTTPSを処理するための各種フィルターを選びます。</li>
  <li>HTTPフィルターにより、リクエストの宛先ポート (例：<code>50010</code>) で、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで管理しています (例：<code>50010</code>) 。</li>
  <li>HTTPフィルターにより、リクエストの宛先ホスト (例：<code>example.com</code>) やパス (例：<code>/</code>) で、Serviceを絞り込みます。Envoyは、クラスターを宛先ポートやホストで管理しています (例：`inbound|50010|foo-service.foo-namespace.svc.cluster.local`) </li>
  <li>エンドポイントを選びます。Envoyは、エンドポイントをローカルホストや宛先ポートで管理しています (例：<code>127.0.0.1:50010</code>) 。
  <li>ローカルホストにリクエストを送信します。結果的に、宛先マイクロサービスに通信が届きます。</li>
</ul>
...
1. 送信元マイクロサービスのリクエストの宛先ポート (例：`443`) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで管理しています (例：`0.0.0.0_443`) 。
2. HTTPSを処理するための各種フィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポート (例：`443`) で、ServiceEntryを絞り込みます。Envoyは、ルートを宛先ポートで管理しています (例：`443`) 。
4. HTTPフィルターにより、リクエストの宛先ホスト (例：`istio-egressgateway-service.foo-namespace.svc.cluster.local`) やパス (例：`/`) で、Serviceを絞り込みます。Envoyは、クラスターをIstio EgressGateway 宛先ポートやホストで管理しています (例：`outbound|443|istio-egressgateway-service.foo-namespace.svc.cluster.local`) 。
...
<ul class="decimal">
  ...
  <li>HTTPSを処理するための各種フィルターを選びます。</li>
  <li>HTTPフィルターにより、リクエストの宛先ポート (例：<code>443</code>) で、ServiceEntryを絞り込みます。Envoyは、ルートを宛先ポートで管理しています (例：<code>50010</code>) 。</li>
  <li>HTTPフィルターにより、リクエストの宛先ホスト (例：<code>external.com</code>) やパス (例：<code>/</code>) で、ServiceEntryを絞り込みます。Envoyは、クラスターを宛先ポートやホストで管理しています (例：<code>outbound|443|external.com</code>) 。</li>
