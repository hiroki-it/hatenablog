...
1. リクエストの宛先ポート (例：`50000`) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで命名します (例：`0.0.0.0_50000`) 。
2. HTTPSを処理するための各種フィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで命名します (例：`50000`) 。
4. HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。これが、クラスター (例：`outbound|50000|foo-service.foo-namespace.svc.cluster.local`) になります。
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、Service配下のPodを選びます。これが、エンドポイント (例：`<PodのIPアドレス>:50000`) になります。
6. パケットのL7ヘッダーを暗号化した上で、リクエストを宛先PodにL7ロードバランシングします。

### 宛先Pod側の`istio-proxy`コンテナ

<ul class="decimal">
  <li value="7">L7ロードバランシングされたリクエストの宛先ポート (例：<code>50000</code>) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで命名します (例：<code>0.0.0.0_50000</code>)</li>
  <li>HTTPSを処理するための各種フィルターを選びます。</li>
  <li>HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで命名します (例：<code>50000</code>) 。</li>
  <li>HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。これが、クラスター (例：<code>inbound|50000|foo-service.foo-namespace.svc.cluster.local</code>) になります。</li>
  <li>Service配下のPodとして、ローカルホスト (例：<code>127.0.0.1:50000</code>) を選びます。
  <li>ローカルホストのマイクロサービスにリクエストを送信します。</li>
</ul>
...
1. リクエストの宛先ポート (例：`50010`) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで命名します (例：`0.0.0.0_50010`) 。
2. HTTPSを処理するための各種フィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで命名します (例：`50010`) 。
4. HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。これが、クラスター (例：`outbound|50010|foo-service.foo-namespace.svc.cluster.local`) になります。
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、Service配下のPodを選びます。これが、エンドポイント (例：`<PodのIPアドレス>:50010`) になります。
6. パケットのL7ヘッダーを暗号化した上で、リクエストを宛先PodにL7ロードバランシングします。

### 宛先Pod側の`istio-proxy`コンテナ

<ul class="decimal">
  <li value="7">L7ロードバランシングされたリクエストの宛先ポート (例：<code>50010</code>) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで命名します (例：<code>0.0.0.0_50010</code>)</li>
  <li>HTTPSを処理するための各種フィルターを選びます。</li>
  <li>HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで命名します (例：<code>50010</code>) 。</li>
  <li>HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。これが、クラスター (例：<code>inbound|50010|foo-service.foo-namespace.svc.cluster.local</code>) になります。</li>
  <li>Service配下のPodとして、ローカルホスト (例：<code>127.0.0.1:50010</code>) を選びます。
  <li>ローカルホストのマイクロサービスにリクエストを送信します。</li>
</ul>
...
1. リクエストの宛先ポート (例：`443`) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで命名します (例：`0.0.0.0_443`) 。
2. HTTPSを処理するための各種フィルターを選びます。
3. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。Envoyは、ルートを宛先ポートで命名します (例：`443`) 。
4. HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。Istio EgressGateway Serviceがクラスター (例：`outbound|443|istio-egressgateway-service.foo-namespace.svc.cluster.local`) になります。
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、Istio EgressGateway Service配下のPodを選びます。これが、エンドポイント (例：`<PodのIPアドレス>:443`) になります。
6. パケットのL7ヘッダーを暗号化した上で、宛先Istio EgressGateway PodにL7ロードバランシングします。

### 宛先Pod (Istio EgressGateway Pod) 側の`istio-proxy`コンテナ

<ul class="decimal">
  <li value="7">L7ロードバランシングされたリクエストの宛先ポート (例：<code>443</code>) で、リスナーを絞り込みます。Envoyは、リスナーを宛先ポートで命名します (例：<code>0.0.0.0_443</code>)</li>
  <li>HTTPSを処理するための各種フィルターを選びます。</li>
  <li>HTTPフィルターにより、リクエストの宛先ポートで、エントリを絞り込みます。これは、ServiceEntryルート (例：<code>50010</code>) になります。</li>
  <li>HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。これが、クラスター (例：<code>inbound|50010|foo-service.foo-namespace.svc.cluster.local</code>) になります。</li>
  <li>Service配下のPodとして、ローカルホスト (例：<code>127.0.0.1:50010</code>) を選びます。
  <li>ローカルホストのマイクロサービスにリクエストを送信します。</li>
</ul>
