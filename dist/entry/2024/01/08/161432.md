...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>TLS Inspector</td>
      <td>EnvoyがTLSを検知できるようにします。<br>通信にTLSを使用している場合、リスナーフィルターの後にTLS Transport socketを実行します。<br>TLS Transport socketは、TLSハンドシェイクを実行し、パケットのL7ヘッダーを復号化します。
    </tr>
    <tr>
      <td style="text-align: center;">ネットワーク<br>フィルター</td>
      <td>HTTP connection manager</td>
      <td>L7ヘッダーを読み取ります。<br>また、後続のHTTPフィルターを制御します。
    </tr>
    <tr>
