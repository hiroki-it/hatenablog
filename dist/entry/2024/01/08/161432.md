...
7. L7ロードバランシングされたリクエストの宛先ポート (例：`50000`) で、リスナー (例：`0.0.0.0_50000`) を絞り込みます。
8. HTTPSを処理するための各種フィルターを選びます。
9. HTTPフィルターにより、リクエストの宛先ポートで、Serviceを絞り込みます。これが、ルート (例：`50000`) になります。
10. HTTPフィルターにより、リクエストの宛先ホスト/パスで、Serviceを絞り込みます。これが、クラスター (例：`inbound|50000|foo-service.foo-namespace.svc.cluster.local`) になります。
11. Service配下のPodとして、ローカルホスト (例：`127.0.0.1:50000`) を選びます
12. ローカルホストのマイクロサービスにリクエストを送信します。
...
5. 設定した負荷分散方式 (例：ラウンドロビン、など) に応じて、Istio EgressGateway Service配下のPodを選びます。これが、エンドポイント (例：`<PodのIPアドレス>:50000`) になります。
