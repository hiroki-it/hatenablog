...
採用した設計プラクティスの紹介も兼ねて、ArgoCDのアーキテクチャとデプロイの仕組みを記事にしました🚀

ArgoCDを使用しない場合、開発者が`kubectl`コマンドや`helm`コマンドで、手動でマニフェストをデプロイしなければなりません。
...
- 下位レイヤー方向にしか依存関係がなく、例えばドメインとインフラの間で依存性は逆転させていない。
- レイヤーの種類と上記の依存関係の方向から、レイヤードアーキテクチャのような構成になっている。
