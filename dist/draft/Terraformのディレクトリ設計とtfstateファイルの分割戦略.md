...
この時、`tfstate`ファイルの分割手法を整理、課題に合った適切な手法を採用することになりました。

今回は、この分割手法を記事で紹介しました。
...
ただし、`terraform_remote_state`ブロックとは異なり、直接的には`tfstate`ファイルに依存しません。

`data`ブロックの場合は、実際のAWSリソースの状態に依存することにより、間接的に他の`tfstate`ファイル依存することになります。
